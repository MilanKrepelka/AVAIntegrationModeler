...
To update the database use this command from the Web project folder (replace `AVAIntegrationModeler` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../AVAIntegrationModeler.Infrastructure/AVAIntegrationModeler.Infrastructure.csproj -s AVAIntegrationModeler.Web.csproj
```
