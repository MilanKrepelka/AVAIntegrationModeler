  ...
  : ICommandHandler<ImportDataModelCommand, Result<Guid>>
{
  public async Task<Result<Guid>> Handle(ImportDataModelCommand request, CancellationToken cancellationToken)
  {
    ...
    var existing = await dataModelRepository.FirstOrDefaultAsync(
      new DataModelByCodeSpec(dto.Code), cancellationToken);

    Guid localModelId;
    if (existing is null)
      localModelId = await CreatePathAsync(dto, cancellationToken);
    else
      localModelId = await UpdatePathAsync(existing, dto, cancellationToken);

    if (localModelId == Guid.Empty)
      return Result<Guid>.Error("Import datového modelu selhal.");

    await ImportRecordsAsync(request.AvaPlaceModelId, localModelId, cancellationToken);
